...
async def retry_with_fallback_models(
    ...
    model_selections: Sequence[ReviewModelSelection] | None = None,
):
    model_selections = tuple(model_selections or ())
    if model_selections:
        all_models = [selection.model for selection in model_selections]
        # The configured fallback deployments belong to the configured model chain.
        # An explicit selector chain replaces that chain, while retaining the primary
        # deployment for installations that require one.
    ...
    for i, (model, deployment_id) in enumerate(zip(all_models, all_deployments)):
        selection = model_selections[i] if model_selections else None
        try:
            get_logger().debug(
